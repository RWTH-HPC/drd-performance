...
#define CLIENT_TOOL_VERBOSE_INIT_VAR OMPT_MULTIPLEX_TOOL_NAME "_VERBOSE_INIT"
#endif
#endif
...
static void* client_tool_module = NULL;
void ompt_multiplex_finalize(ompt_data_t *fns) {
  ...
  if(client_tool_module)
    dlclose(client_tool_module);
}
...
ompt_start_tool_result_t *ompt_start_tool(unsigned int omp_version,
  ...
  if (tool_libs) {
    ...
    size_t progress = 0, tmp_progress = -1, len = strlen(tool_libs);
    // Reset dl-error
    dlerror();
    void *h = NULL;
    OMPT_VERBOSE_INIT_PRINT(
        "Look for candidates within " CLIENT_TOOL_LIBRARIES_VAR "...\n");
    while (tmp_progress + 1 < len) {
      // skipping ":" here allows to use continue
      progress = ++tmp_progress;
      if(h) { // close library from previous iteration
        dlclose(h);
      }
      ...
      if (h) {
        OMPT_VERBOSE_INIT_CONTINUED_PRINT("Sucess.\n");
        OMPT_VERBOSE_INIT_PRINT("Looking for ompt_start_tool ... ");
        client_start_tool =
            (ompt_start_tool_result_t * (*)(unsigned int, const char *))
                dlsym(h, "ompt_start_tool");
        if (!client_start_tool) {
          OMPT_VERBOSE_INIT_CONTINUED_PRINT("Failed with: %s\n", dlerror());
          continue;
        }
        OMPT_VERBOSE_INIT_CONTINUED_PRINT("Sucess.\n");
        OMPT_VERBOSE_INIT_PRINT("Calling ompt_start_tool ... ");
        if ((ompt_multiplex_client_fns =
  ...
  OMPT_VERBOSE_INIT_PRINT("ompt_multiplex_client_fns = %p\n",
