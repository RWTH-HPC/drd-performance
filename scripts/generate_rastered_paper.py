    ...
    for a in self.applications.values():
      if (apps=="all" or a.name in apps):
        ...
        with plt_backend.PdfPages(output_path+"/output_"+str(excl_or_incl)+"_"+a.name+"_cache_usage.pdf", keep_empty=False) as pdf:
          ...
          print("Printing region ...", end=' ', flush=True)
          for r_name in a.regions.keys():
            print(r_name, end=' ', flush=True)
            r = a.regions[r_name]
            fig_pie, axes2 = plt.subplots(2,4,subplot_kw=dict(polar=True), figsize=(8,5)) # polar plot in background to draw axes onto
...
#            fig_pie.legend(labels=labels_pie,loc='upper center',bbox_to_anchor=(0.5,-0.2),ncol=2)
            ...
            axl = fig_pie.add_subplot(241,frameon=False)
            axl.set_xticks([])
            axl.set_yticks([])
#            axl.visible=False
#            ax = axes2[0]
