...
TINY_SIZE = 6
SMALL_SIZE = TINY_SIZE+1
MEDIUM_SIZE = TINY_SIZE+1
LARGE_SIZE = TINY_SIZE+2
...
class Measurement():
    ...
    for a in self.applications.values():
      if (apps=="all" or a.name in apps):
        ...
        with plt_backend.PdfPages(output_path+"/output_"+excl_or_incl+"_"+a.name+"_region_overhead.pdf") as pdf:
          ...
          print("Printing counters ...", end=' ', flush=True)
          for c in self.counters:
            ...
            for variant in self.variants:
              if (variant != "base"):
                ...
                a.draw_region_overheads(pdf,title,width,self.tnums,variant,variant_idx,base_idx,ctr_idx,meas_idx,excl_or_incl)
          print(" ... done")
    ...
    for a in self.applications.values():
      if (apps=="all" or a.name in apps):
        ...
        with plt_backend.PdfPages(output_path+"/output_"+str(excl_or_incl)+"_"+a.name+"_counter_overhead.pdf") as pdf:
          self.draw_app_region_times(pdf, excl_or_incl, a.name)
          print("Printing regions ...", end=' ', flush=True)
          for r_name in a.regions.keys():
            print(r_name, end=' ', flush=True)
            for variant in self.variants:
  ...
  def draw_app_region_times(self,pdf,excl_or_incl,app):
    ...
    fig, axes = plt.subplots(len(self.variants),1,figsize=(10,len(self.variants)*1.2+.5))
...
#    fig.legend(*axes[0].get_legend_handles_labels(), bbox_to_anchor=(0,1,1,1), loc="lower left", mode="expand")
    axes[6].set_xticklabels(a.regions.keys(), rotation=20, ha="right")
    pdf.savefig()
    plt.close()
        ...
        with plt_backend.PdfPages(output_path+"/output_"+str(excl_or_incl)+"_"+a.name+"_cache_usage.pdf", keep_empty=False) as pdf:
          for r_name in a.regions.keys():
            for variant in self.variants:
              ...
              variant_idx = self.variant_to_index(variant)
              base_idx = self.variant_to_index("base")
              a.draw_cache_usages(pdf,title,self.counters,width,self.tnums,variant_idx,base_idx,meas_idx,r_name,excl_or_incl)
...
class Application():
  ...
  def draw_region_overheads(self,pdf,title,width,tnums,variant,variant_idx,base_idx,ctr_idx,meas_idx,excl_or_incl):
    labels = self.regions.keys()
    cmap = plt.get_cmap("Blues")
    ...
    if(np.any(usable_values)):
      ...
      fig, ax = plt.subplots()
      ax2 = ax.twinx()
      ax2_label_variant = variant#"variant"
      ax2_label_base = "base"
...
#        if (ax2_label_variant == "variant"):
      ...
      ax.set_title("B. "+title, pad=30.0, fontdict={'fontweight' : 'bold','fontsize' : LARGE_SIZE+1})
      ax.set_xticks(x)
...
#        ax_pie2.xaxis.set_major_locator(mticker.FixedLocator([0.,.875,.75,.625,.5,.375,.25,.125]))
#        ax_pie2.xaxis.set_major_locator(mticker.FixedLocator([0.,.75,.5,.25]))
  ...
  def draw_counter_overheads(self,pdf,title,counters,width,tnums,variant_idx,base_idx,meas_idx,r_name,excl_or_incl):
    ...
    if(np.count_nonzero(values_base) and np.count_nonzero(values_variant)):
      ...
      ax.set_ylabel("Relative value")
      # ax.set_yscale("log")
      ax.set_title("A. Comparing "+title+" variant\nwith base for region "+r_name, pad = 30.0, fontdict={'fontweight' : 'bold','fontsize' : LARGE_SIZE+1})
      ax.set_xticks(x)
...
def add_cutoff_flags(ax,bars,cutoff,nbars,i_bar):
  ...
  for i,bar in enumerate(bars):
    if (bar.get_height() > cutoff):
      ...
      at.get_bbox_patch().set_color("black")
      at.get_bbox_patch().set_facecolor(bar.get_facecolor())
      at.get_bbox_patch().set_linewidth(.5)
...
#meas_obj.draw_region_overheads(path,"excl","351.bwaves")
meas_obj.draw_region_overheads(path,"excl","all")
meas_obj.draw_counter_overheads(path,"excl","all")
